(function () {
  ...
  function renderMeetingsPanel(isIsmso) {
    ...
    const showPublishedAccessMeeting = [
      'access-meetings-program-officer',
      'checkin-document-study-team',
      'checkin-document-program-officer',
      'checkin-document-dsmb-member'
    ].includes(state.activeTrainingId);
  ...
  function renderDocumentsPanel() {
    const sim = state.simulation;
    const checkInOutTraining = [
      'checkin-document-chair',
      'checkin-document-study-team',
      'checkin-document-program-officer',
      'checkin-document-dsmb-member'
    ].includes(state.activeTrainingId);
  ...
  function agendaActions(count) {
    return `
      <div class="agenda-actions">
        <button type="button" class="icon-action" data-target="attendance-capture">Capture Attendance${count ? `<b>${escapeHtml(count)}</b>` : ''}</button>
        <button type="button" class="icon-action">Manage Participants</button>
        <button type="button" class="icon-action">Comments</button>
      </div>
    `;
  }
    ...
    return `
      <div class="detail-subsection">
        ...
        ${cromsTable({
          ...
          rows: [
            ['Training DSMB Virtual Meeting Notice', 'reviewer-a@example.org; reviewer-b@example.org', '07/10/2026 02:06 PM ET', '<button type="button" class="icon-action">Download</button>'],
            ['DSMB Meeting Materials Now Available in CROMS', 'member.group@example.org', '07/10/2026 12:39 PM ET', '<button type="button" class="icon-action">Download</button>'],
            ['DSMB Meeting Materials Now Available in CROMS', 'chair.example@example.org', '07/10/2026 12:33 PM ET', '<button type="button" class="icon-action">Download</button>'],
            ['DSMB Meeting Materials Now Available in CROMS', 'program.official@example.org', '07/10/2026 12:21 PM ET', '<button type="button" class="icon-action">Download</button>']
          ]
        })}
      </div>
      <div class="form-actions nav-actions">
      ...
      <div class="screen-header details-page-header">
        <div class="upper d-flex">
          ...
          <div class="screen-actions">
            ...
            <button type="button" class="kebab-large">More Actions</button>
          </div>
        </div>
        <ul class="h-list bullets">
          ${meta.map(([label, value]) => `
  ...
  function kebabButton() {
    return '<button type="button" class="row-kebab">Actions</button>';
  }
